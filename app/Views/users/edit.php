...
<div class="toolbar">
    <div class="toolbar__left">
        <a class="btn btn--secondary btn--sm" href="<?= site_url('users/' . $uid) ?>"><?= esc(lang('App.users_back_user')) ?></a>
    </div>
</div>

<div class="card" style="max-width: 880px;">
    <h2 class="card__title"><?= esc(lang('App.users_edit_title')) ?></h2>
    <?= form_open(site_url('users/' . $uid . '/update'), ['class' => 'stack']) ?>
        <div class="form-grid">
            <div class="field" style="grid-column: 1 / -1;">
                <label for="display_name"><?= esc(lang('App.users_form_display_name')) ?> <span aria-hidden="true">*</span></label>
                <input class="input" id="display_name" name="display_name" type="text" required value="<?= esc(old('display_name', (string) ($user['display_name'] ?? '')), 'attr') ?>" autocomplete="name">
            </div>
            <div class="field" style="grid-column: 1 / -1;">
                <label for="email"><?= esc(lang('App.users_form_email')) ?> <span aria-hidden="true">*</span></label>
                <input class="input" id="email" name="email" type="email" required value="<?= esc(old('email', (string) ($user['email'] ?? '')), 'attr') ?>" autocomplete="email">
            </div>
            <div class="field">
                <label for="password"><?= esc(lang('App.users_form_password_optional')) ?></label>
                <input class="input" id="password" name="password" type="password" autocomplete="new-password">
            </div>
            <div class="field">
                <label for="password_confirm"><?= esc(lang('App.users_form_password_confirm_optional')) ?></label>
            ...
            <div class="field" style="grid-column: 1 / -1;">
                ...
                <label class="checkbox-label" style="display:flex;align-items:center;gap:0.5rem;">
                    <?php $activeDefault = (int) ($user['is_active'] ?? 0) === 1 ? '1' : '0'; ?>
                    <input type="checkbox" name="is_active" value="1" <?= ((string) old('is_active', $activeDefault)) === '1' ? 'checked' : '' ?>>
                ...
                <div class="stack" style="gap:0.35rem;">
                    <?php foreach ($assignableRoles as $role) : ?>
                        ...
                        if (is_array($oldRoles)) {
                            $checked = in_array($slug, $oldRoles, true);
                        } else {
                            $checked = in_array($slug, $checkedSlugs, true);
                        }
                        ?>
                        <label class="checkbox-label" style="display:flex;align-items:center;gap:0.5rem;">
        ...
        <div class="toolbar" style="margin: 0; padding-top: 0.5rem;">
            ...
            <div class="toolbar__right">
                <a class="btn btn--secondary" href="<?= site_url('users/' . $uid) ?>"><?= esc(lang('App.action_cancel')) ?></a>
