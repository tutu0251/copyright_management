...
<p class="page-intro">Create a reusable owner record. You can link it to one or more works with percentage and role from each work’s ownership screen.</p>

<div class="card" style="max-width: 880px;">
    <h2 class="card__title">Owner details</h2>
    <?= form_open(site_url('owners'), ['class' => 'stack']) ?>
        <div class="form-grid">
            <div class="field">
            ...
            <div class="field">
                ...
                <select class="select" id="owner_type" name="owner_type" required>
                    <?php foreach ($types as $val => $label) : ?>
                        <option value="<?= esc($val, 'attr') ?>" <?= old('owner_type', 'individual') === $val ? 'selected' : '' ?>><?= esc($label) ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div class="field">
        ...
        <div class="field">
            ...
            <textarea class="textarea" id="notes" name="notes" rows="3" placeholder="Internal notes (not shown on public pages)"><?= esc(old('notes', '')) ?></textarea>
        </div>
        <div class="toolbar" style="margin: 0; padding-top: 0.5rem;">
            <div class="toolbar__left"></div>
            <div class="toolbar__right">
                <a class="btn btn--secondary" href="<?= site_url('owners') ?>">Cancel</a>
